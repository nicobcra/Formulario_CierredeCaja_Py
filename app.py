...
# HELPERS PIJAMAS
# ─────────────────────────────────────────
def obtener_productos_pijamas(tienda_id):
    rows = sb_get("productos", {
        "select":    "*,proveedores(nombre),producto_tallas(id,talla,stock)",
        "tienda_id": f"eq.{tienda_id}",
        "activo":    "eq.true",
        "order":     "nombre.asc",
    })
    result = []
    for r in rows:
        tallas = sorted(r.get("producto_tallas") or [], key=lambda t: t["talla"])
        ...
        result.append({
            ...
            "proveedor":    (r.get("proveedores") or {}).get("nombre",""),
            "precio_compra": formatear_cop(r.get("precio_compra",0) or 0),
            "precio_venta":  formatear_cop(r.get("precio_venta",0) or 0),
            "precio_venta_raw": r.get("precio_venta",0) or 0,
            "stock_total":  stock_total,
            "stock_bajo":   stock_total <= r.get("stock_minimo", 3),
            "tallas":       tallas,
        })
    return result
...
# ─────────────────────────────────────────
# RUTAS PIJAMAS — PÁGINAS
# ─────────────────────────────────────────
@app.route("/inventario-pijamas")
@login_requerido
def inventario_pijamas():
    tienda_id = session["tienda_id"]
    productos = obtener_productos_pijamas(tienda_id)
    return render_template("inventario_pijamas.html",
        modulo="inventario",
        productos=productos,
        proveedores=obtener_proveedores(tienda_id),
        total_productos=len(productos),
...
@app.route("/ventas-pijamas")
@login_requerido
def ventas_pijamas():
    tienda_id = session["tienda_id"]
    productos = obtener_productos_pijamas(tienda_id)
    return render_template("ventas_pijamas.html",
        modulo="ventas",
        productos=productos,
    )
...
# RUTAS PIJAMAS — API
# ─────────────────────────────────────────
@app.route("/guardar-producto-pijama", methods=["POST"])
@rol_requerido("dueno", "admin")
def guardar_producto_pijama():
    try:
        data      = request.json
        tienda_id = session["tienda_id"]

        ok, res = sb_post("productos", {
            "nombre":        data.get("nombre","").strip(),
            ...
            "stock_minimo":  int(data.get("stock_minimo", 3)),
            "activo":        True,
        })
        if not ok:
            return jsonify({"ok": False, "error": res}), 500
...
@app.route("/actualizar-talla-stock", methods=["POST"])
@rol_requerido("dueno", "admin")
def actualizar_talla_stock():
    try:
        data      = request.json
        ...
        rows = sb_get("producto_tallas", {
            "id":        f"eq.{talla_id}",
            "tienda_id": f"eq.{tienda_id}",
            "select":    "stock",
        })
        if not rows:
            return jsonify({"ok": False, "error": "Talla no encontrada"}), 404

        nuevo = max(0, rows[0]["stock"] + delta)
        ok, err = sb_patch("producto_tallas", f"id=eq.{talla_id}", {"stock": nuevo})
        if not ok:
            return jsonify({"ok": False, "error": err}), 500

        return jsonify({"ok": True, "stock": nuevo})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/registrar-venta-pijama", methods=["POST"])
@login_requerido
def registrar_venta_pijama():
    try:
        data      = request.json
        tienda_id = session["tienda_id"]
        metodo    = data.get("metodo", "")
        ...
        for item in items:
            ...
            if talla_id:
                rows = sb_get("producto_tallas", {
                    "id": f"eq.{talla_id}", "select": "stock"
                })
                if rows:
                    nuevo = max(0, rows[0]["stock"] - item.get("cantidad", 1))
                    sb_patch("producto_tallas", f"id=eq.{talla_id}", {"stock": nuevo})

        resumen = ", ".join(
            f'{i["nombre"]} T:{i["talla"]} x{i["cantidad"]}' for i in items
        )
        return jsonify({
            "ok":     True,
            "total":  total,
